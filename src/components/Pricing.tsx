...
import { Check, Info, Calculator, ShieldCheck, Percent, HelpCircle } from 'lucide-react'
...
export const Pricing: React.FC = () => {
  ...
  const [marketplaceCommission, setMarketplaceCommission] = useState<number>(15) // IFood typically takes 12-23%
  ...
  const duliviFee = billingCycle === 'monthly' ? 139.90 : 139.90 * discountFactor
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-slate-500 mt-3 text-sm md:text-base leading-relaxed">
            Planos flexíveis sem multas de cancelamento que crescem junto com o seu delivery.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch mb-20">
          {plans.map((plan) => {
            const currentPrice = billingCycle === 'monthly' ? plan.priceMonthly : plan.priceMonthly * discountFactor
            return (
              <div
                key={plan.id}
                  ...
                  <div className="my-6">
                    ...
                    {billingCycle === 'annual' && (
                      <span className="text-[10px] font-semibold text-emerald-600 block mt-1">
                        Equivalente a R$ {(plan.priceMonthly * 12 * discountFactor).toFixed(2).replace('.', ',')} por ano
                      </span>
                    )}
                  </div>
                  ...
                  <ul className="space-y-3">
                    {plan.features.map((feat, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-xs text-slate-700 font-medium leading-relaxed">
                        <Check size={14} className="text-blue-600 flex-shrink-0 mt-0.5" strokeWidth={3} />
                        <span>{feat}</span>
                      </li>
                    ))}
                  </ul>
        ...
        <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6 md:p-10 text-white grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative overflow-hidden">
          ...
          <div className="lg:col-span-5 space-y-5 relative">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              Diferente de iFood e Rappi, a Dulivi cobra apenas uma <b>mensalidade fixa</b>. Simule seus ganhos abaixo e veja o lucro retornar para o seu caixa.
            </p>
            ...
            <div className="space-y-4 pt-3">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">Ticket Médio por Pedido</span>
                  <span className="font-mono text-blue-400 font-extrabold">R$ {ticketSize}</span>
                </div>
                <input 
                  type="range" 
                  min="20" 
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">Pedidos Concluídos por Mês</span>
                  <span className="font-mono text-blue-400 font-extrabold">{ordersPerMonth} pedidos</span>
                </div>
                <input 
                  type="range" 
                  min="50" 
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">Taxa cobrada pelo marketplace atual</span>
                  <span className="font-mono text-blue-400 font-extrabold">{marketplaceCommission}%</span>
                </div>
                <input 
                  type="range" 
                  min="8" 
          ...
          <div className="lg:col-span-7 bg-slate-950/40 rounded-2xl border border-slate-800/80 p-5 md:p-6 space-y-6 relative">
            
            {/* Split Metrics */}
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800/60">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Faturamento Mensal</span>
              ...
              <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800/60">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block text-rose-400">Taxas pagas hoje</span>
                <span className="text-sm md:text-base font-black text-rose-400 font-mono block mt-1">
                  R$ {marketplaceFees.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
            </div>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex gap-4 items-center">
              ...
              <div>
                <span className="text-[10px] text-slate-300 font-bold block uppercase tracking-wider">Economia com Dulivi Pro</span>
                <span className="text-2xl font-black text-emerald-400 font-mono leading-none mt-1 block">
                  R$ {monthlySavings.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-[10px] text-slate-400 font-bold">/ mês</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-6 text-xs text-slate-400 justify-between items-start sm:items-center pt-2">
              <div className="flex items-center gap-1.5">
                ...
                <span>Economia anual estimada de <b className="text-emerald-400 font-mono">R$ {annualSavings.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></span>
              </div>
              <a 
                href="#contact-form-section" 
