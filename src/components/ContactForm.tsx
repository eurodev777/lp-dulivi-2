...
export const ContactForm: React.FC<ContactFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(
        "Sucesso!",
        "Seu delivery foi configurado com sucesso.",
        "success",
      );
    ...
    >
      <div className="mb-6">
        ...
        <p className="text-xs md:text-sm text-slate-500 mt-2 leading-relaxed">
          Preencha os dados abaixo para gerar seu cardápio instantâneo e obter
          acesso ao gestor completo de pedidos.
        </p>
      </div>
          ...
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
              <CreditCard
                size={16}
                className={
                  focusedField === "cpf" ? "text-blue-500" : "text-slate-400"
                }
              />
            </div>
            <input
