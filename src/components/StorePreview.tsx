...
export const ACCENT_THEMES: Record<AccentColorKey, { name: string; bg: string; text: string; accent: string }> = {
  dulivi: { name: 'Dulivi Blue', bg: '#1d84ff', text: '#ffffff', accent: '#0c62e6' },
  emerald: { name: 'Verde Fresh', bg: '#10b981', text: '#ffffff', accent: '#047857' },
  amber: { name: 'Laranja Hot', bg: '#f59e0b', text: '#ffffff', accent: '#b45309' },
  violet: { name: 'Roxo Premium', bg: '#8b5cf6', text: '#ffffff', accent: '#6d28d9' },
  rose: { name: 'Rosa Saboroso', bg: '#f43f5e', text: '#ffffff', accent: '#be123c' },
}
...
export const StorePreview: React.FC<StorePreviewProps> = ({ formData }) => {
  ...
  const storeName = formData.name?.trim() || 'Pizzaria Forno de Ouro'
  ...
  const menuItems: MenuItem[] = [
    {
      ...
      description: 'Dois blends suculentos de 120g, cheddar duplo cremoso, cebola grelhada e molho artesanal.',
      price: 28.90,
      category: 'Burger',
      image: '🍔'
    },
    {
      ...
      description: 'Coxa e sobrecoxa de frango desossada e empanada, alface crespa, maionese verde e pão brioche.',
      price: 24.90,
      category: 'Burger',
      image: '🍗'
    },
    {
      ...
      description: 'Molho de tomate artesanal, muçarela, calabresa fatiada, cebola roxa e o legítimo Catupiry.',
      price: 42.00,
      category: 'Pizzas',
      image: '🍕'
    },
    {
      ...
      description: 'Muçarela premium, provolone, parmesão ralado e gorgonzola cremosa com orégano fresco.',
      price: 48.00,
      category: 'Pizzas',
      image: '🧀'
    },
    {
      ...
      description: 'Suco natural feito de laranjas selecionadas, gelado e refrescante.',
      price: 8.50,
      category: 'Bebidas',
      image: '🍹'
    },
  ...
  const totalItems: number = (Object.values(cart) as number[]).reduce((sum: number, count: number) => sum + count, 0)
  const totalPrice: number = (Object.entries(cart) as [string, number][]).reduce((sum: number, [id, count]: [string, number]) => {
  ...
  return (
    <div className="relative mx-auto max-w-[340px] w-full" id="store-preview-wrapper">
      {/* Decorative Badge */}
      <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 flex items-center gap-1.5 bg-slate-900/95 backdrop-blur-md text-white text-xs px-4 py-2 rounded-full shadow-lg border border-white/10 z-10 whitespace-nowrap">
      ...
      <div className="relative border-[10px] border-slate-900 rounded-[44px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] bg-slate-50 overflow-hidden w-full aspect-[9/19.5]">
        ...
        <div className="absolute top-6 left-0 right-0 h-6 px-6 flex justify-between items-center text-[10px] font-bold text-slate-700 z-30 select-none">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[9px] bg-slate-200 px-1 py-0.5 rounded text-slate-600 font-mono">5G</span>
        ...
        <div className="absolute inset-0 pt-12 pb-14 overflow-y-auto no-scrollbar flex flex-col bg-white">
          ...
          {/* Header Banner */}
          <div 
            className="p-5 pt-6 text-white transition-all duration-500 relative overflow-hidden"
          ...
          >
            ...
            <div className="relative flex justify-between items-start">
              <div>
                <span className="inline-block text-[9px] font-extrabold uppercase bg-white/20 backdrop-blur-md text-white px-2 py-0.5 rounded-full mb-1.5 tracking-wider">
                  Aberto • Até 23:59
                </span>
          ...
          <div className="bg-amber-50 border-b border-amber-100 p-3 px-4 flex items-center gap-2 text-[11px] text-amber-800 font-medium">
            <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 bg-amber-100 text-amber-700 rounded-full">🔔</span>
            <span className="truncate">Peça pelo WhatsApp com cupom <b>PRIMEIRO10</b>!</span>
          </div>
                      ...
                      {qtyInCart > 0 ? (
                        ...
                        >
                          ...
                          <button 
                            ...
                            className="w-5 h-5 rounded flex items-center justify-center text-white cursor-pointer"
                            style={{ backgroundColor: activeTheme.bg }}
                          >
                        ...
                        <button
                          ...
                          className="w-6 h-6 rounded-lg flex items-center justify-center text-white shadow-2xs hover:scale-105 active:scale-95 transition-all cursor-pointer"
                          style={{ backgroundColor: activeTheme.bg }}
                        >
          ...
          <div className="p-6 border-t border-slate-50 bg-slate-50/50 text-center text-[10px] text-slate-400 select-none">
            ...
            <p className="mt-0.5">Powered by <span className="font-semibold text-slate-500">Dulivi</span></p>
          </div>
        </div>
          ...
          {totalItems > 0 && !orderSent && (
            ...
            >
              <button
                ...
                className="w-full h-11 rounded-xl text-xs font-extrabold text-white flex items-center justify-between px-4 shadow-lg hover:shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer"
                style={{ backgroundColor: activeTheme.bg }}
              >
        ...
        <AnimatePresence>
          {orderSent && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-950/90 z-50 flex flex-col items-center justify-center p-6 text-center text-white"
            >
              <motion.div
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="w-16 h-16 bg-emerald-500 text-white rounded-full flex items-center justify-center text-2xl shadow-lg shadow-emerald-500/20 mb-4"
              >
              ...
              <p className="text-[11px] text-slate-300 mt-2 leading-relaxed max-w-[200px] mx-auto">
                No app real, Dulivi formata e envia uma mensagem perfeita diretamente no seu WhatsApp do restaurante!
              </p>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-left font-mono text-[9px] text-slate-400 mt-5 w-full max-w-[240px] space-y-1 select-all">
                <p className="text-emerald-400 font-bold">💬 Mensagem enviada:</p>
                <p>--- NOVO PEDIDO ---</p>
                <p><b>Estabelecimento:</b> {storeName}</p>
                <p><b>Itens:</b></p>
                {Object.entries(cart).map(([id, qty]) => {
                  const item = menuItems.find(i => i.id === id)
                  return <p key={id}>- {qty}x {item?.name}</p>
                })}
                <p><b>Total:</b> R$ {totalPrice.toFixed(2).replace('.', ',')}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
              ...
              >
                ...
                <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                  ...
                  <button
                    ...
                    className="h-9 px-5 rounded-lg text-xs font-bold text-white shadow-xs flex items-center gap-1 cursor-pointer"
                    style={{ backgroundColor: activeTheme.bg }}
                  >
